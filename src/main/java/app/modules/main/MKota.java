...
public class MKota {
    ...
    public static Kota create(Map map) {
        ...
        try {
            ...
            ZHelper.logInfo(MKota.class, ZHelper.getPesan());
        } catch (DBException e) {
            ZHelper.logInfo(MKota.class, e.getMessage());
        }
        return cat;
    }

    /**
     * Read all records in Kota
     *
     * @return LazyList<Kota>
     */
    public static LazyList<Kota> ReadAll() throws DBException {
        return Kota.findAll();
    }

    /**
     * Read record in Kota by ID
     *
     * @param ID Object
     * @return Kota
     */
    public static Kota ReadByID(Object ID) throws DBException {
        return Kota.findById(ID);
    }

    public static Kota ReadByName(Object NAME) throws DBException {
        Kota cats = new Kota();
        LazyList<Kota> cat = cats.find("keterangan = ?", NAME);
        for (Kota mList : cat) {
            cats = mList;
        }
        return cats;
    ...
    public static String getKetByID(Object ID) throws DBException {
        return Kota.findById(ID).getString("keterangan");
    }

    /**
     * Update Kota Record with Map<String,String>
     *
     * @param map map with attributes to overwrite this models'. Keys are names of attributes of this model, values
    ...
    public static boolean update(Map map, Long ID) throws DBException {
        ...
        try {
            Kota cat = Kota.findById(ID);
            cat.fromMap(map);
            result = cat.saveIt();
            ZHelper.logInfo(MKota.class, "Update Data id : " + ID.toString());
        } catch (DBException e) {
            ZHelper.logInfo(MKota.class, e.getMessage() + "Update Ke modul DB gagal");
        }
        return result;
    }

    public static boolean delete(Object ID) throws DBException {
        return Kota.findById(ID).delete();
    }
    ...
    public static boolean delete(String subquery, Object params) {
        boolean result = false;
        try {
            LazyList<Kota> mm = Kota.where(subquery, params);
            for (Kota mList : mm) {
                result = mList.delete();
            }
